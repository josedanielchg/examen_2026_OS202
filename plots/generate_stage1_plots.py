...
def write_interactive_scaling_csv() -> None:
    baseline = STAGE1_INTERACTIVE_SCALING[0][3]
    path = ROOT / "stage1_interactive_scaling.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["threads", "render_ms", "update_ms", "total_ms", "speedup", "efficiency"])
        for threads, render_ms, update_ms, total_ms in STAGE1_INTERACTIVE_SCALING:
...
def write_interactive_breakdown_csv() -> None:
    path = ROOT / "stage1_interactive.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["label", "time_ms"])
        writer.writerow(["Render", f"{STAGE1_INTERACTIVE_SCALING[2][1]:.3f}"])
        writer.writerow(["Update", f"{STAGE1_INTERACTIVE_SCALING[2][2]:.3f}"])
...
def write_interactive_breakdown_plot() -> None:
    render_ms = STAGE1_INTERACTIVE_SCALING[2][1]
    update_ms = STAGE1_INTERACTIVE_SCALING[2][2]
    content = f"""
\\begin{{tikzpicture}}
\\begin{{axis}}[
    ybar,
...
\\end{{tikzpicture}}
""".strip() + "\n"
    (ROOT / "stage1_interactive_plot.tex").write_text(content, encoding="utf-8")


def write_interactive_speedup_plot() -> None:
    baseline = STAGE1_INTERACTIVE_SCALING[0][3]
    coords = "\n".join(
        f"        ({threads}, {baseline / total_ms:.3f})"
...
"""
    ...
    (ROOT / "stage1_interactive_speedup_plot.tex").write_text(content, encoding="utf-8")


def write_interactive_efficiency_plot() -> None:
    baseline = STAGE1_INTERACTIVE_SCALING[0][3]
    coords = "\n".join(
        f"        ({threads}, {(baseline / total_ms) / threads:.3f})"
        for threads, _render_ms, _update_ms, total_ms in STAGE1_INTERACTIVE_SCALING
    )
    content = (
        r"""
...
"""
    ...
    (ROOT / "stage1_interactive_efficiency_plot.tex").write_text(content, encoding="utf-8")
...
def main() -> None:
    ...
    write_initial_csv()
    write_scaling_csv()
    write_interactive_scaling_csv()
    write_interactive_breakdown_csv()
    write_initial_plot()
    write_speedup_plot()
    write_efficiency_plot()
    write_interactive_breakdown_plot()
    write_interactive_speedup_plot()
    write_interactive_efficiency_plot()
