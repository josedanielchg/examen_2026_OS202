...
Cette étape vise à exploiter le parallélisme mémoire partagée disponible sur la machine locale. La machine de test dispose d'un processeur AMD Ryzen 7 5800H avec 8 cœurs physiques et 16 threads logiques. La campagne de mesure utilise \(1, 2, 4, 8, 16\) threads.

\subsection{Changements clés d'implémentation}

\begin{itemize}
    \item une nouvelle version du script a été créée dans \path{stages/01_numba_threads/} pour conserver l'étape de référence intacte ;
    \item les boucles indépendantes ont été parallélisées avec \texttt{Numba} et \texttt{prange}, en particulier pour le calcul des accélérations, les masses/centres de masse des cellules et l'intégration de Verlet ;
    \item les parties sensibles aux \textit{race conditions}, comme le comptage et l'insertion des corps dans les cellules, ont été conservées en série ;
    \item un mode \texttt{--benchmark} sans affichage a été ajouté pour mesurer uniquement le coût du calcul.
\end{itemize}

\subsection{Principes de parallélisation retenus}

Dans cette étape, les choix retenus sont les suivants :
\begin{itemize}
